...
const (
	...
	//数据库表前缀 compass100表示当前应用版本是compass:1.0.x版，数据库结构发生变化必然是中型升级
	TABLE_PREFIX = "compass10_"
...
grant all privileges on compass.* to compass identified by 'compass123';
flush privileges;
*/
/*
 你也可以在运行时的参数中临时修改一些配置项：
-MysqlHost=127.0.0.1 -MysqlPort=3306 -MysqlSchema=compass -MysqlUsername=compass -MysqlPassword=compass123
*/
var (
	CONFIG = &Config{
		//以下内容是默认配置项。
		...
		AdminUsername: "admin",
		//超级管理员邮箱
		AdminEmail: "admin@compass.eyeblue.cn",
		//超级管理员密码
...
//第一级. 从配置文件conf/compass.json中读取配置项
func LoadConfigFromFile() {
	//读取配置文件
	filePath := GetConfPath() + "/compass.json"
